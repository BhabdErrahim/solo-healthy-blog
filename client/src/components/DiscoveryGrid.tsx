...
export default function DiscoveryGrid({ articles }: { articles: any[] }) {
  ...
  // Safeguard: Ensure we don't crash if articles array is empty
  const mainArticle = articles?.[0];
  const mainImageUrl = mainArticle ? getFullImageUrl(mainArticle.thumbnail) : null;
  ...
  return (
    <section className="max-w-7xl mx-auto py-24 px-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 h-auto md:h-[600px]">
        {/* Large Bento Item */}
        <Link href={`/article/${mainArticle?.slug}`} className="md:col-span-2 md:row-span-2 relative rounded-[2.5rem] overflow-hidden group bg-gray-100">
          ...
          <div className="absolute bottom-8 left-8 right-8 text-white">
            <h3 className="text-2xl font-bold">{mainArticle?.title}</h3>
          </div>
        </Link>

        {/* Small Bento Items */}
        {articles.slice(1, 5).map((article) => {
          const thumbUrl = getFullImageUrl(article?.thumbnail);
          
          return (
            <Link key={article.id} href={`/article/${article.slug}`} className="relative rounded-[2rem] overflow-hidden group h-64 md:h-full bg-gray-100">
